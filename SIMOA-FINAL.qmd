...
```{r}
#| label: Categorical Tables

# Load required libraries
library(dplyr)
library(kableExtra)
library(flextable)
library(officer)  # for fp_border function

# Initialize results dataframe (place this before your loop)
results <- data.frame(
  Variable = character(),
  Type = character(),
  Effect_Size = numeric(),
...
# After your loop, create the APA table
create_apa_odds_table <- function(results_df) {
  # Clean and format the results
  table_data <- results_df %>%
    mutate(
      # Format OR with 2 decimal places
      OR_formatted = sprintf("%.2f", OR_or_Mean_Diff),
      ...
      # Clean variable names (remove underscores, capitalize)
      Variable_clean = gsub("_", " ", Variable),
      Variable_clean = tools::toTitleCase(Variable_clean)
    ) %>%
    select(Variable_clean, OR_formatted, CI_formatted, P_formatted)
  
  # Create the table using kableExtra
  apa_table <- table_data %>%
    kbl(
      col.names = c("Variable", "Odds Ratio", "95% CI", "p"),
      align = c("l", "c", "c", "c"),
      caption = "Table 1. Odds Ratios for Categorical Variables Associated with BZRA Discontinuation"
    ) %>%
    kable_styling(
...
# Alternative function using flextable (better for Word documents)
create_apa_odds_flextable <- function(results_df) {
  # Clean and format the results
  table_data <- results_df %>%
    mutate(
      # Format OR with 2 decimal places
  ...
  ft <- flextable(table_data) %>%
    ...
    autofit() %>%
    add_footer_lines("Note. BZRA = Benzodiazepine receptor agonist; CI = confidence interval. Odds ratios > 1 indicate higher odds of discontinuation.")
...
# 1. Create and display the table using kableExtra
apa_table_kable <- create_apa_odds_table(results)
apa_table_kable  # Display the table (don't use print())

# 2. Create flextable version (better for Word output)
apa_table_flex <- create_apa_odds_flextable(results)
apa_table_flex  # Display the flextable

# 3. Save to Word document
library(officer)
doc <- read_docx() %>%
  body_add_flextable(apa_table_flex)
print(doc, target = "Categorical Variable Table.docx")

# 4. Save as HTML
save_kable(apa_table_kable, "Categorical Variable Table.html")

# 5. For LaTeX/PDF output
library(knitr)

# First create the formatted data
table_data <- results %>%
  mutate(
    OR_formatted = sprintf("%.2f", OR_or_Mean_Diff),
    CI_formatted = sprintf("%.2f, %.2f", CI_Lower, CI_Upper),
...
# Then create the LaTeX table
kable(table_data, format = "latex", booktabs = TRUE, 
      col.names = c("Variable", "Odds Ratio", "95% CI", "p"),
      caption = "Odds Ratios for Categorical Variables Associated with BZRA Discontinuation")

# Additional formatting function for complex tables
format_complex_apa_table <- function(results_df) {
  # Group by variable type if needed
  table_data <- results_df %>%
    arrange(Variable) %>%
    mutate(
      OR_CI = sprintf("%.2f [%.2f, %.2f]", OR_or_Mean_Diff, CI_Lower, CI_Upper),
      P_formatted = case_when(
        P_Value < 0.001 ~ "< .001",
      ...
      Effect_Size_Note = case_when(
        Effect_Magnitude == "Large" ~ "**",
        Effect_Magnitude == "Medium" ~ "*",
        TRUE ~ ""
      )
    ) %>%
    select(Variable_clean, OR_CI, P_formatted, Effect_Size_Note)
  
  # Create table with effect size indicators
  ft <- flextable(table_data) %>%
    set_header_labels(
      Variable_clean = "Variable",
      OR_CI = "OR [95% CI]",
    ...
    autofit() %>%
    add_footer_lines(c(
      "Note. BZRA = Benzodiazepine receptor agonist; CI = confidence interval; OR = odds ratio.",
